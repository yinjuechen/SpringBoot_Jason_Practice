...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	public void configure(HttpSecurity http) throws Exception {
		http.cors().and().csrf().disable()
				...
				// .and()
				.authorizeRequests()
				.antMatchers(HttpMethod.GET, "/index.html", "/trucks", "/myusers", "/*").permitAll()
				.antMatchers(HttpMethod.POST,"/users","/products", "/*").permitAll()
//				.antMatchers(HttpMethod.GET, "/applications", "/applications/*").authenticated()
//				.antMatchers(HttpMethod.GET, "/applications").hasAnyRole("ADMIN", "AGENT").anyRequest().authenticated()
				.and()
				.exceptionHandling().authenticationEntryPoint(authenticationEntryPointImpl)
				.accessDeniedHandler(accessDeniedHandlerImpl).and().formLogin().usernameParameter("email")
